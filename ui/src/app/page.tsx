...
export default function WorkspacePage() {
  return (
    <div className="glow-panel mx-auto flex max-w-4xl flex-col gap-8 px-6 py-10">
      <section className="rise-in">
        ...
        <p className="mt-3 max-w-xl text-[13px] text-ink-1">
          ...
          условия, география, период. Ответ — с доказательной базой,
          провенансом до цитаты и проверкой каждого числа.
        </p>
      </section>
      ...
      <section className="rise-in" style={{ animationDelay: "40ms" }}>
        <form action="/" className="flex gap-2">
          ...
          <button
            type="submit"
            className="h-12 rounded-sm bg-electrolyte px-5 font-medium text-bg-0 transition-colors hover:bg-electrolyte/90"
          >
            Спросить
          </button>
        </form>
      ...
      >
        ...
        <p>
          База знаний пуста — загрузите документы в разделе «Документы» или
          дождитесь индексации корпуса. Пресеты выше заработают сразу после
          коммита первых фактов.
        </p>
      </section>
    </div>
  );
...
function PresetCard({ preset, index }: { preset: Preset; index: number }) {
  return (
    <a
      href={`/?q=${encodeURIComponent(preset.question)}`}
      className="rise-in group flex flex-col gap-2 rounded-sm border border-line bg-bg-1 p-4 transition-all hover:-translate-y-px hover:border-line-strong hover:bg-bg-2"
