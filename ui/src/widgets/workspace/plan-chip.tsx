    ...
    <div className="rise-in rounded-sm border border-line-strong bg-bg-1 px-4 py-3">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        ...
        {plan.paramConstraints.map((constraint) => (
          <span
            key={constraint.parameter.slug}
            className="inline-flex items-center gap-1.5 rounded-sm border border-line bg-bg-2 px-2 py-1 font-mono text-[11px] text-ink-1"
          >
            {constraint.parameter.name} {constraint.value.operator}{" "}
            {constraint.value.vmin}
