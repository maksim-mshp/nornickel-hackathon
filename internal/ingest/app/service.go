...
func (service *Service) ListDocuments(ctx context.Context, offset uint32, limit uint32) ([]DocumentSummary, uint32, error) {
	if limit == 0 {
		limit = 50
	}
	if limit > 100 {
	...
	return service.repository.ListDocuments(ctx, offset, limit)
}
