...
Один `task demo` поднимает всё; профили compose: `core` (обязательное), `llm-local` (vLLM, только при наличии GPU), `obs` (наблюдаемость). Настройки — в `configs/demo/*.yml` (upstream LLM: `llm-routes.yml`; `parse.ocr: off`; `embed.backend: remote` и т.д.); ключи — в `configs/secrets.yml` (в .gitignore); каталог `configs/` монтируется в контейнеры read-only. `.env` не используется вовсе. Авторизация в демо — `auth.mode: demo` (статические токены-роли, полный RBAC/RLS без внешнего IdP, [03-architecture.md](03-architecture.md) ADR-6); Keycloak — в прод-контуре.
