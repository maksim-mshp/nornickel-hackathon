...
**Вычислительные ресурсы организаторами не выдаются — разворачиваем сами** (инцидент 04.07: у части команд забрали и Yandex-доступ, и серверы). Контур укладывается в ориентир проверяющих (это не выдаваемый стенд, а «железо жюри»): **≤1 GPU ≤16 ГБ, 12 CPU, 32 ГБ RAM**; Docker/Compose/Helm допустимы, если образы доступны во внешних репозиториях. Жюри обязано получить **рабочую ссылку** на развёрнутое решение — при самостоятельной сборке жюри и ошибке сборки баллы снижают вплоть до 0, поэтому рабочий деплой + записанный скринкаст готовим сами.
...
│                                 # + secrets.yml (ключи; в .gitignore, шаблон secrets.yml.example)
...
├── deploy/
│   ├── compose/                  # dev/demo: docker-compose.yml, профили core|llm-local|obs
│   └── vm/                       # прод на VM: compose.prod.yml, caddy/, backup/, systemd/, deploy.sh
...
Один `task demo` поднимает всё; профили compose: `core` (обязательное), `llm-local` (vLLM, только при наличии GPU), `obs` (наблюдаемость). Настройки — в `configs/demo/*.yml` (upstream LLM: `llm-routes.yml`; `parse.ocr: off`; `embed.backend: remote` и т.д.); ключи — в `configs/secrets.yml` (в .gitignore); каталог `configs/` монтируется в контейнеры read-only. `.env` не используется вовсе. Авторизация в демо — `auth.mode: demo` (статические токены-роли, RBAC/RLS без внешнего IdP; **устаревшее требование ТЗ — demo-режим as-is, без развития**, [03-architecture.md](03-architecture.md) ADR-6); Keycloak — в прод-контуре.
...
- **Эмбеддинги — remote** (`embed.backend: remote` в `configs/demo/embed.yml`): bge-m3 + rerank через DO Gradient ($0.02/$0.01 за 1M, проверено 02.07) — веса не грузятся, embed-контейнеру хватает 256 МБ; офлайн-фолбэк — `onnx-int8` (+~2 ГБ RAM).
...
### 3.1. Состав (deploy/vm/compose.prod.yml)
...
- **Конфигурация**: `configs/prod/*.yml` (в git, ревьюится как код); **секреты** — только `configs/secrets.yml` вне git (mode 600 на VM, доставляется деплой-скриптом отдельно от репо); ключи LLM — только там.
...
| Auth | `auth.mode: demo` (токены-роли) | `auth.mode: hybrid` (demo-токены + Keycloak/OIDC, composite verifier; `configs/demo/gateway.yml`) | Keycloak/OIDC (`auth.mode: oidc`) + корп. IdP federation позже |
