...
- **Секреты**: только `configs/secrets.yaml` вне git (mode 600 на VM); ротация ключей LLM API.
...
- Конфигурация: **только YAML** (`configs/base` + оверлей окружения + `configs/secrets.yaml`, ревью как код, валидация при старте); env-переменные не используются; фиче-флаги — поля YAML; без внешней конфиг-системы.
