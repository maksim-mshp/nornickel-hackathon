...
- OCR выключен (текстовый слой гарантирован организаторами), включается в YAML-конфиге (`parse.ocr: auto`, Docling OCR-профиль) — деградация скорости, не архитектуры; **таблицы (TableFormer) — первоклассный источник; значимая информация встречается также в схемах и изображениях** — извлечение из них (VLM-описания картинок → в chunks/факты) подключается флагом парсера как доп. возможность, не ядро;
...
**Мультиядерность parse/extract:** оба воркера — pull-консьюмеры JetStream с пулом из N asyncio-задач (N = `parse.workers`/`extract.workers`, по умолчанию `os.cpu_count()`), тяжёлая CPU-работа (Docling-парсинг pdf/docx/pptx/xlsx/xls; numcore) выносится в `ProcessPoolExecutor` — параллелизм по документам на всех ядрах. extract использует `forkserver`-контекст пула (воркеры форкаются из чистого процесса без gRPC).

**LLM/эмбеддинги в extract:** после numcore extract обогащает bundle через gRPC — эмбеддинги чанков от `kmap-embed` (bge-m3 на DO, батчи `extract.embed_batch`) кладутся в `chunks[].embedding`; сущности предметной области — от `kmap-llm` (задача `extract_entities`, Yandex→DigitalOcean-фолбэк, JSON) добавляются в `entities[]` (slug `<etype>:<slugify(name_en|name)>`). Оба вызова best-effort: при недоступности сервиса extract логирует и продолжает (граф degradation — документ всё равно доходит до `indexed` с numcore-фактами). Адреса — `extract.llm_addr`/`extract.embed_addr` в `configs/base/common.yml`. Реранк bge-reranker-v2-m3 (DO) используется на этапе запроса (search), не в extract. Каждая стадия перед работой проверяет наличие своего артефакта в MinIO (parse — `kmap-docir/<doc>/docir.json`, extract — `kmap-bundles/<doc>/bundle.json`) и делает ACK без работы, если он уже есть → повторный прогон/replay дешёвый и обрабатывает только незавершённые документы (идемпотентность §2). Лимиты CPU/RAM пула — в `deploy/compose` (`parse`/`extract`: `cpus`, `mem_limit`); при нехватке RAM снизить `parse.workers`.
