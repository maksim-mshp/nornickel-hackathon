...
| **«Реестр моделей» организаторов = Yandex AI Studio** | выдан 03.07 (это и есть доступ от организаторов) | API-ключ + folder_id, **1 на команду**, номинально без лимитов, модели из registry + эмбеддинги, прямого UI нет. **Инцидент в ночь 04.07:** из-за циклической нагрузки и перерасхода бюджета доступ к Yandex-моделям ограничили, у части команд забрали серверы → критична деградация на фолбэк DO Gradient (§1.3) и способность работать на CPU |
...
|---|---|---|---|
| R1 | Датасет хакатона (**~5 ГБ, Яндекс.Диск `disk.yandex.ru/d/npigiuw4Rbe9Pg`**): корпус отчётов/статей (Word/PDF с текстовым слоем + онлайн-ресурсы с картинками; **RU/EN/китайский/испанский**; значимая инфо в таблицах/схемах/изображениях), каталог экспериментов, справочники, сотрудники/лаборатории, таксономия тегов. Папка **«обзоры» = эталонный образ результата** (ручная работа команд экспертов; по титульному листу восстанавливается исходный вопрос). Внешние данные **разрешены и приветствуются**. Складывается в `data-sources/` (в .gitignore) | ingestion, gold-set v1, калибровка numcore, эталон для eval | выдан 03.07 |
...
| R3 | Хост/VM для демо (8 vCPU/32 ГБ) либо договорённость «демо с ноутбука» — **серверы организаторами не выдаются (инцидент 04.07 подтвердил), разворачиваем сами**; жюри нужна рабочая ссылка (иначе баллы вплоть до 0) | стабильный показ жюри | день 2 |
...
1. ~~Состав «реестра моделей»~~ — **закрыт:** это Yandex AI Studio (§1): open-source модели из registry + эмбеддинги, 1 ключ на команду, прямого UI нет.
2. Объём/состав датасета — **частично закрыт:** ~5 ГБ, языки RU/EN/китайский/испанский, папка «обзоры» = эталон. Остаётся: реальные ли ФИО сотрудников (PII-режим expert-слоя).
3. Формат каталога экспериментов (колонки CSV/XLSX) и справочников — для маппингов ingestion.
4. ~~Формат сдачи решения~~ — **закрыт:** VCS + облачный диск (архив исходников + видео-демо ≤5 мин) + презентация + ссылка на развёрнутое решение ([14-roadmap.md](14-roadmap.md) §3).
5. ~~Как проходит демо~~ — **закрыт:** только своё развёрнутое решение (серверы не выдаются), рабочая ссылка + обязательный видео-скринкаст.
6. Критерии оценки — **частично раскрыты** (качество/содержательность ответов, продуктовая ценность для учёного, архитектура, ML-культура; размер команды учитывается), точные веса публично не раскрывают.
7. ~~Допустим ли routerai.ru~~ — **правилами разрешён**; остаётся решить, покидают ли данные датасета периметр (иначе — только vLLM on-prem).
...
- Ориентир «железа жюри» (НЕ выдаваемый стенд, решение разворачиваем сами): ≤1 GPU ≤16 ГБ, 12 CPU, 32 ГБ RAM; Docker/Compose/Helm допустимы, если образы доступны во внешних репозиториях.
- Дедлайн загрузки: **04.07 23:59 МСК**, де-факто правки ссылок принимают до **06:00 05.07**; сдача — раздел «Загрузка решения» на платформе.

## 5. Контакты и полезные ссылки трека (справочно)

- **Лидер трека:** Светлана Нападыханова (гл. менеджер Департамента научно-технического развития, Институт Гипроникель — бизнес-сторона, ИС).
- **Эксперты:** Антон Абрамов (ИИ/бизнес), Максим (ИИ-эксперт, преподаватель ЦУ — ведёт технические разъяснения).
- **Модераторы чата трека:** Кирилл `@sergeenkokirill`, Настя `@semitsvetikn`, Лера `@leravalera3`; личные вопросы — `@nornickel_hack_support`; оператор хакатона — PG (`ask@pgenesis.ru`).
- **Рекомендованные внешние источники** (для расширения корпуса): researchgate.net, elibrary.ru, link.springer.com, patents.google.com, mdpi.com, cyberleninka.ru, onlinelibrary.wiley.com, sciencedirect.com, sci-hub.ru.
