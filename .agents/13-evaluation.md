...
Разметка извлечения: `eval/gold/numeric_facts.yml` — ≥150 числовых фактов из ≥20 документов разных типов (протоколы, статьи RU/EN/китайский/испанский, таблицы); `entities.yml` — ≥100 сущностей; `relations.yml` — ≥40 связей; `contradiction_pairs.yml` — ≥40 пар (положительные/отрицательные, включая «разные условия — не противоречие»).
