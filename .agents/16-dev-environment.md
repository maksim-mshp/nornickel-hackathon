...
- Базовые Go-библиотеки платформы: `koanf/v2` — загрузка YAML-конфигурации из `configs/base` + оверлеев `configs/<env>` + `configs/secrets.yml`, выбран вместо самописного merge из-за готовой поддержки provider/parser/strict merge; `chi/v5` — HTTP routing для gateway, health/readiness и будущих REST-хендлеров, выбран как уже зафиксированный в архитектуре лёгкий роутер, совместимый с `net/http`; `grpc-go` — внутренний gRPC runtime, стандартный health service и reflection для диагностики через `grpcurl`; `go-grpc-middleware/v2` — готовые gRPC interceptors, сейчас recovery от panic, дальше auth/logging/metrics без смены runtime; `google/uuid` — генерация request_id при отсутствии `x-request-id` в metadata; `slog` — стандартный structured logging без внешней зависимости.
- Dependency-first правило: перед добавлением самописного слоя проверить готовое решение и использовать его, если оно зрелое, сопровождаемое и не конфликтует с архитектурой. Повторяющиеся задачи не пишутся вручную: routing, REST↔gRPC glue, validation, retry/backoff, config merge, SQL-access, migrations, mocks, codegen, auth, metrics/tracing, S3/NATS/PG-интеграции и UI-примитивы закрываются библиотеками или генераторами. Самописный код допустим как тонкая glue-логика вокруг выбранного инструмента, доменная логика или осознанное исключение; исключение фиксируется в `.agents/` рядом с решением.
