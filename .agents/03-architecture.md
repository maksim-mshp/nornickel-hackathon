...
| **Identity & Access** | OIDC-аутентификация, RBAC, уровни доступа документов, аудит | kmap-gateway + Keycloak (прод) / `auth.mode: demo` (демо) | Principal, Grant, AuditRecord |
...
### ADR-6. Авторизация: RBAC + RLS в MVP (решение владельца, 02.07.2026, по итоговому ТЗ)
Итоговое ТЗ содержит раздел «Управление доступом и безопасность» (роли, разграничение доступа, аудит) — реализуем в MVP. **AuthN**: OIDC (Keycloak 26.6) в проде — валидация JWT (JWKS-кэш), маппинг claims → `Principal{user_id, roles[], doc_access}`; для демо — статические токены-роли (`auth.mode: demo` в YAML) — те же роли без внешнего IdP. **AuthZ**: middleware RBAC по таблице «роль → операции» ([01-brief.md](01-brief.md) §2.2) + **Row-Level Security в PG** на уровни доступа документов `public/internal/confidential/restricted` — фильтрация в самом сторе (политики на `core.documents`, каскадно на факты/чанки через `document_id`); principal-контекст передаётся вниз в gRPC-метаданных и устанавливается в сессии (`SET LOCAL app.*`). Агрегаты (coverage) считаются без раскрытия закрытых текстов — счётчики, не содержимое.
...
- bge-m3 — единое кросс-языковое векторное пространство: русский запрос находит английский документ без перевода.
- FTS: два generated-столбца `tsv_ru` (config `russian`) и `tsv_en` (config `english`), поиск по обоим с весами.
